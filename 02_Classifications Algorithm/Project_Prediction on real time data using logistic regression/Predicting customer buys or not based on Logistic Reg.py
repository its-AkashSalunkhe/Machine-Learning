...
##############################       Making real world prediction on future data ( validation Data )      ####################################



# Here we have a dataset that contains customer info, based on which we have to decide whether the customer will buy (purchase) a product or not.
# To make the predictions we are going to use LOGISTIC REGRESSION Algorithm.
# The data we have is seprate we just used here with LOGISTIC REGRESSION model to predict its future.


data1 = pd.read_csv(r"C:\Users\Akash\Desktop\FSDS\ML\Classifications Algorithm\Prediction on real time data using logistic regression\Future_prediction1.csv")
data1
...
y_pred1 = pd.DataFrame()           # Creating an Empty DataFrame (Table)


d2['y_pred1'] = classifier.predict(m)

d2.to_csv('your_predictions.csv')
